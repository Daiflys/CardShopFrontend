...
const SearchGridCard = ({ card, onClick, formatPrice, getAvailableCount }) => {
  ...
  // Get the SearchGridCard component from current skin
  const SearchGridCardComponent = useComponent('SearchGridCard');
  ...
  // If no SearchGridCardComponent is loaded yet, show a fallback
  if (!SearchGridCardComponent) {
    return (
      <div
        ...
        onClick={onClick}
      >
        <div className="aspect-[3/4] bg-gray-50 rounded-t-lg p-2 sm:p-3">
  ...
  const actionSection = hasNMStock ? (
    <>
    ...
    <Button
      ...
      className="w-full py-2 px-2 sm:px-3 text-xs sm:text-sm"
    >
    ...
    <Button
      ...
      className="w-full py-2 px-2 sm:px-3 border-red-400 text-red-600 hover:bg-red-50 text-xs sm:text-sm flex items-center justify-center gap-1 sm:gap-2"
    >
      <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
