...
const getBanners = (t) => [
  {
    id: 1,
    title: t('banners.aetherDrift.title', 'Aether Drift'),
    subtitle: t('banners.aetherDrift.subtitle', 'Explore new planes and discover powerful cards'),
    cta: t('banners.aetherDrift.cta', 'Explore Set'),
  ...
  {
    id: 2,
    title: t('banners.finalFantasy.title', 'Final Fantasy'),
    subtitle: t('banners.finalFantasy.subtitle', 'Epic battles await with legendary characters'),
    cta: t('banners.finalFantasy.cta', 'Discover Cards'),
...
const BannerNew = () => {
    ...
    nextSlide.innerHTML = `
      <div class="absolute inset-0 bg-gradient-to-br ${nextBanner.bgColor}">
        ...
        <div class="absolute inset-0 h-full w-full">
          <div class="h-full w-full bg-cover bg-center" style="background-image: url(${nextBanner.image});">
  ...
  return (
    <section className="relative overflow-hidden px-4 md:px-6 lg:px-8 mt-6 w-full">
      ...
      <div className="relative w-full h-80 md:h-96 lg:h-[450px] overflow-hidden rounded-2xl group">

        {/* Left arrow - only visible on hover */}
        <button
          onClick={handlePrev}
          className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 rounded-full p-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          aria-label="Previous"
        >
        ...
        <button
          onClick={handleNext}
          className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 rounded-full p-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          aria-label="Next"
        >
          <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          {banners.map((_, idx) => (
            <button
              ...
              className={`w-4 h-4 rounded-full transition-all duration-300 flex-shrink-0 ${
                current === idx ? "bg-sky-400" : "bg-white/60"
              }`}
              style={{
                width: '16px',
                height: '16px',
