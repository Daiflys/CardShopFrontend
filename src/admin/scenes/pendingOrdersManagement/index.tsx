...
const PendingOrdersManagement: React.FC = () => {
  ...
  // Load pending orders (AWAITING_VENDOR_CONFIRMATION)
  const loadOrders = async (): Promise<void> => {
    setLoading(true);
    setError(null);

    try {
      const response = await getPurchasesByStatus("AWAITING_VENDOR_CONFIRMATION", page, pageSize);
  ...
  const handleConfirmOrder = async (): Promise<void> => {
    ...
    try {
      await confirmPurchaseAdmin(selectedOrder.id);
    ...
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header
          ...
          subtitle="Review and manage orders awaiting vendor confirmation"
        />
        <Box>
      ...
      <Grid container spacing={2} mb={3}>
        <Grid item xs={12} md={4}>
          ...
          >
            <HourglassEmptyOutlinedIcon sx={{ fontSize: 40, color: colors.blueAccent[500] }} />
            <Box>
